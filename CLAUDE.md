...
17. **Subitem e alternativa vão em lista Markdown, um por linha iniciada por `- `** — as alternativas `a) b) c) d)` das `OBJ` e as listas `a) b) c)` de especificação das demais questões, com linha em branco antes do primeiro item e nenhuma entre eles. Como linhas soltas de prosa, a lista se funde num parágrafo único ao renderizar **ou ao colar no Google Docs**, que é como a folha chega ao papel. Parágrafo separado por linha em branco resolve a renderização e não resolve a colagem; a lista resolve as duas. Aplicado às 1.056 alternativas das 47 folhas em 14/08.
