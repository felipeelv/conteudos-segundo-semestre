...
**Um registro de método, da rodada de 14/08 — o rótulo `### ATIVIDADE NN`.** As 814 questões trocaram o `**N.**` grudado no enunciado por um rótulo em linha própria. Duas coisas que a rodada ensinou:

- **A numeração já reiniciava por capítulo, e não era corrida por folha.** A premissa contrária sobreviveu meia conversa antes de a contagem desmenti-la: 27 das 47 folhas reiniciam, e as outras 20 só parecem corridas porque têm um capítulo só. A grade referencia a questão pelo par **(Cap, Q)** — preservar o número como estava manteve as 12 grades válidas sem tocar em nenhuma linha delas.
- **Reformatação em lote se verifica contra `git HEAD`, não contra o relato do script que a fez.** O conversor conferiu a si mesmo e passou; a checagem que vale foi a segunda, independente, comparando cada folha com a versão anterior em sete pontos — sequência de números, zero-padding, resíduo do padrão antigo, linha em branco após o rótulo, quebras duras preservadas, texto byte-idêntico fora do rótulo, e nenhum `###` antes do primeiro `##`. É o mecanismo que o §9.1 já prescrevia.
...
| 3 | `ATIVIDADES/METODOLOGIA/06-PRODUCAO.md` | **o arquivo operacional**: marcadores, pool, sorteio, travas T1–T9, banco de verbos (§6), catálogo de **60 itens em 19 tipos** (§5), molde de saída (§7, com o **rótulo `### ATIVIDADE NN`**, o molde de `OBJ` e a calibragem por faixa etária), as **14 checagens** (§8) — com §8.1 independência, §8.2 acessibilidade, §8.3 execução individual e §8.4 qualidade do distrator |
...
12. **Cada questão abre com o rótulo `### ATIVIDADE NN` em linha própria** (§7.1) — dois dígitos com zero à esquerda, numeração reiniciando a cada capítulo, enunciado no parágrafo seguinte. A hierarquia da folha é `#` ano · `##` capítulo · `###` atividade. O par (Cap, Q) da grade continua sendo a chave de referência. Aplicado às 814 questões das 47 folhas em 14/08.
13. **A folha separa fazer de entregar** (§7.1). Enunciado-síntese → execução em prosa compacta → `Responda:` em pergunta → `Confira você mesmo:`. Nunca etapas rotuladas ("Etapa 1 — a malha"), nunca a questão inteira em lista plana. **Marcador só em dois casos:** (a) sub-sequência cuja ordem importa — a construção com régua e compasso, a sequência de teclas da calculadora; (b) especificações paralelas de um mesmo produto — as colunas de uma tabela, os elementos que um mapa precisa ter. O marcador lista requisito verificável, nunca entrega o artefato montado.
14. **O `Responda:` admite complemento de localização e abre nos dois tempos** (§7.1). `Responda, abaixo dos gráficos:` quando a questão gera produto na página; e pergunta que comprime dois raciocínios se desdobra — *apontar o fator que explica a maior diferença* vira *qual é a maior diferença, e que fator a explica?*. Ao abrir o segundo tempo, a linha da grade acompanha na mesma passada.
...
16. **O gabarito das `OBJ` vive na grade do `_ORGANIZACAO.md`, nunca na folha.** A linha traz a letra e o motivo de eliminação esperado de cada distrator — é a eliminação que decide 8 dos 10 pontos de R14.
17. **Rode as 14 checagens de `06-PRODUCAO.md` §8** antes de passar ao capítulo seguinte. A 14ª é a dos distratores (§8.4): erro plausível, motivo próprio para cada um, comprimento parecido, e nada de *"todas as anteriores"*.
18. **Subitem e alternativa ocupam uma linha cada, terminada em dois espaços** — a quebra dura do Markdown, aplicada às alternativas `a) b) c) d)` das `OBJ` e às listas `a) b) c)` de especificação das demais questões, em todas menos a última do bloco, que a linha em branco seguinte fecha. **Sem marcador `- ` antes da letra**: lista imprime `• a)`, redundante numa alternativa já rotulada. Sem os dois espaços, o bloco se funde num parágrafo único ao renderizar **e ao colar no Google Docs**, que é como a folha chega ao papel. ⚠️ Os dois espaços são invisíveis: editor que apara espaço no fim da linha desfaz isso em silêncio. Aplicado às 1.056 alternativas das 47 folhas em 14/08.
