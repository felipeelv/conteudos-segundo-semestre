...
17. **Subitem e alternativa vão um por parágrafo, separados por linha em branco** — as alternativas `a) b) c) d)` das `OBJ` e as listas `a) b) c)` de especificação das demais questões, inclusive o primeiro item em relação ao texto que o antecede. Sem a linha em branco o Markdown funde a lista num parágrafo único e o aluno recebe tudo colado. Corrigido nas 47 folhas em 14/08; a regra está em `06-PRODUCAO.md` §7.1 e no molde de `OBJ`.
