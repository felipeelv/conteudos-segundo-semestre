# Ciências — 6º Ano

> **Bloco 1 · 05/08–25/08** · Células e diversidade dos seres vivos
> 2 capítulos · 13 questões.

---

## Capítulo 1 — A célula, unidade da vida

**1.** Ordene, numa linha do tempo, os quatro marcos estudados da descoberta da célula — 1665, 1676, 1838–1839 e 1855 —, nomeando em cada um quem respondeu por ele. Entre cada par vizinho de marcos, escreva uma frase explicando o que a descoberta anterior tornou possível.

**2.** Elabore uma tabela que compare célula animal e célula vegetal. Além das estruturas que as distinguem, inclua uma coluna que relacione cada diferença ao modo de vida do organismo.

**3.** Elabore uma história em quadrinhos de quatro quadros que mostre uma célula por dentro. Ao longo dos quadros precisam aparecer, nomeadas e com a função escrita, pelo menos seis estruturas da célula; no terceiro quadro precisa estar correta a diferença entre a membrana plasmática e a parede celular.

**4.** Escreva um verbete de até cinco linhas sobre a relação entre a forma e a função de uma célula, usando obrigatoriamente os termos **neurônio**, **hemácia** e **cloroplasto**. Em cada um dos três, explique o que aquela célula deixaria de conseguir fazer se tivesse a forma de uma das outras.

**5.** Um estudante escreveu: *"Bactéria é um ser vivo mais simples, então ela é uma célula incompleta."* Identifique o erro e explique por que uma célula procarionte realiza todas as funções vitais sozinha.

**6.** Uma bactéria e uma célula do seu braço são as duas células vivas. Decida qual das duas você chamaria de *ser vivo*, apresentando o critério que aplicou, e explique o que muda quando uma célula passa a integrar um organismo multicelular.

**7.** Arranque duas folhas parecidas de uma mesma planta — de um vaso, de um jardim, de uma praça ou do pátio da escola. Deixe uma sobre a mesa, ao ar, e ponha a outra com o cabo dentro de um copo com água. Antes de começar, escreva o que espera encontrar em cada uma delas no dia seguinte. Observe as duas no dia seguinte e registre o que mudou em cada uma. Compare o que observou com a sua previsão e explique o resultado pela função do vacúolo e da parede celular.

---

## Capítulo 2 — Níveis de organização dos seres vivos

**1.** Elabore um esquema dos níveis de organização, da célula ao organismo, usando um exemplo real do corpo humano em cada nível. Cada seta precisa de um rótulo dizendo o que se ganha ao subir de nível.

**2.** Elabore uma tabela com três sistemas do corpo humano em que fique visível a dependência entre eles: o que cada um fornece aos outros dois e o que recebe deles.

**3.** Explique a um colega que faltou à aula, em um parágrafo de até seis linhas, todos os níveis de organização envolvidos no ato de levantar o braço, do tecido ao sistema.

**4.** Escreva um verbete de até cinco linhas explicando por que subir de nível de organização não é apenas juntar mais peças, usando obrigatoriamente os termos **tecido**, **órgão** e **sistema**. Em cada um dos três, diga o que passa a ser possível nesse nível e não era possível no nível anterior.

**5.** Um estudante escreveu: *"Se uma célula do coração morre, o coração para de funcionar."* Identifique o erro e explique por que a organização em tecidos torna o órgão mais resistente que uma célula sozinha.

**6.** Localize, em duas fontes diferentes — livro, revista, o acervo da biblioteca da escola ou a internet —, quantas células aproximadamente tem o corpo humano adulto e quantos tipos diferentes de célula existem. Registre o autor e a data de cada fonte e explique por que os dois números que você encontrou não são iguais.
